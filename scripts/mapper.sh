...
if [ "${MODE}" = "--uninstall" ]; then
  ...
  # 移除 ~/.agents/skills/ 中指向本 Repo 的 symlink
  ...
  # 移除 ~/.codex/AGENTS.md（僅當內容是本腳本產生的）
...
Available globally via \`\$skill_name\` invocation (symlinked in ~/.agents/skills/):

| \$ Prefix | Role |
|---|---|
| \`\$supervisor\` | Orchestrator & PM |
| \`\$sa\` | System Architect |
| \`\$ui\` | UI/UX Designer |
...
| \`\$sre\` | SRE & Performance |
| \`\$watcher\` | Quality Watcher |
| \`\$logger\` | Technical Writer |
...
AGENTS_EOF
  ...
  echo "✅ 已產生全域指令檔 → ${CODEX_DIR}/AGENTS.md"
fi
