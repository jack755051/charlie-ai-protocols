...
echo "   ✓ Shell  → cap entry 已寫入 ${SHELL_RC}（裸 claude / codex 維持原生 provider）"
...
echo "    cap codex   透過 CAP wrapper 啟動 Codex 並自動記錄 trace"
echo "    cap claude  透過 CAP wrapper 啟動 Claude 並自動記錄 trace"
echo "    cap update  同步 GitHub 最新規則"
echo ""
echo "ℹ Provider Isolation：裸 claude / codex 維持原生 provider 行為（不經 CAP）。"
echo "  CAP-managed session 請走 cap claude / cap codex；若需要 CAP 自動包裹"
echo "  裸 CLI 並記錄 trace（舊行為），請用 CAP_WRAP_NATIVE_CLI=1 make install。"
echo "=========================================="
