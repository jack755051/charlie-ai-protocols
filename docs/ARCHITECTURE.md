...
| `schemas/workflows/` | Supervisor (01) / Engine | 可重複使用的 workflow 模板與流程定義 |
| `schemas/capabilities.yaml` | Engine / Registry | Capability contract 定義（取代舊版 `docs/policies/capability-contracts.md`） |
| `schemas/handoff-ticket.schema.yaml` | Supervisor (01) | 任務交接單的結構化 schema |
...
| `.cap.project.yaml` | 專案識別設定（`project_id`） | CAP runtime / 未來 GUI / OpenClaw |
| `.cap.agents.json` | Agent registry（alias -> provider / prompt / cli） | CAP runtime / 未來 GUI / OpenClaw |
...
- **全數 PASS**：Logger 寫入 `CHANGELOG.md`，並將 trace / devlog / 報告存入 `~/.cap/projects/<project_id>/`，准予進入下一模組。
