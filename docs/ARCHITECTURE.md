...
| `workflow_loader.py` | 載入 workflow YAML，從 `capabilities.yaml` 解析 capability 契約，從 `.cap.agents.json` 查找 agent binding，建構執行計畫 |
...
| 檔案 | 職責 | Runtime 讀取者 |
|---|---|---|
| `schemas/capabilities.yaml` | Capability 契約 SSOT：語意描述、預設 agent、允許 agent、inputs/outputs、完成條件 | `workflow_loader.py` |
| `.cap.agents.json` | Agent binding：alias → prompt_file / provider / cli | `workflow_loader.py`、`cap-registry.sh` |
| `schemas/workflows/*.yaml` | 流程編排：step 順序、依賴、失敗路由、品質門禁 | `workflow_loader.py` |
