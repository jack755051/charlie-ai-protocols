...
> 流水線步驟定義見 [docs/workflows/README.md](./workflows/README.md) 與 `schemas/workflows/` 現役模板。
...
> 品質門禁與 phase 定義以 `schemas/workflows/` 現役模板為準；目前主要收斂在 `version-control-private.yaml`、`readme-to-devops.yaml` 與 `workflow-smoke-test.yaml`。

### Handoff Ticket 欄位參考

Handoff ticket 是任務交接單的概念結構，供 agent prompt 引用（engine 尚未實例化）。核心欄位：

| 欄位 | 說明 |
|---|---|
| `target_capability` | 此任務所需的 capability |
| `task_objective` | 精確描述任務範圍 |
| `workflow_context` | workflow_id / phase / step_id / route_back_to |
| `rules_to_load` | 應掛載的 agent-skills 與 strategy 路徑 |
| `acceptance_criteria` | 完成條件（映射自 step.done_when） |
| `context_payload` | 上游產物路徑（ba_spec_path、api_spec_path 等） |
| `governance` | watcher/security/qa/logger 介入要求 |
| `handoff_output` | agent 完成後填寫：agent_id / task_summary / output_paths / result |

> 原路徑：`schemas/handoff-ticket.schema.yaml`，已於 v0.10.1 降級為概念參考。
