...
- [x] 實作 security checkpoint runner
  - 交付物：security gate runtime
  - 驗收：高風險變更可觸發 security review
  - 進度：done in `v0.22.0` (in-progress)；新增 `engine/security_gate_runner.py` 提供純函式 `run_security_gate()` 與四種機械檢查（`artifact_exists` / `artifact_non_empty` / `secret_patterns` / `risky_keywords`），並在 `engine/step_runtime.py` 新增 `run-security-gate` 子命令。Producer 完整沿用 P8 #1 watcher runner 的 emit-then-self-validate 模式：記憶體先驗 + 落地後重讀再驗，雙重 schema gate 共用 `_validate_gate_result_payload` helper。**Pattern bank**：5 條 secret regex（AWS access key `AKIA…` → critical；private key block `-----BEGIN [A-Z ]*PRIVATE KEY` → critical；generic `api_key/secret_key/...` 賦值 → high；JWT-style 三段式 token → high；hardcoded `password=…` → high）+ 3 條 risky keyword（`dangerouslySetInnerHTML` / `v-html=` → high；`eval(` → medium）。**Severity 策略對齊 08-security-agent.md**：critical → `fail_routing.action=halt`（secret leakage 不允許 escalate 等待）；high-only → `fail_routing.action=escalate`；medium → `result=warn`；missing artifact → `blocked/high/halt`；empty file → `low`（security tolerates empty，與 watcher 的 medium 不同）。**Envelope 邊界**：每檔 finding cap = 10 防止 envelope 爆炸，超過時補一條 `findings_capped` info finding 並建議外接 SAST；scan size cap = 2 MiB 拒絕 binary blob。新增 `tests/scripts/test-security-gate-runner.sh` 覆蓋 13 cases / 74 assertions（clean / AWS key / private key / api_key / dangerouslySetInnerHTML / v-html / eval / missing artifact / empty file / 空 target_artifacts / 預設 `--output` / 11 個 envelope 全部通過 round-trip validate-gate-result CLI / `--produced-by` 與 `--gate-subtype` 覆寫），並 wire 進 `smoke-per-stage.sh`。**邊界**：runner 是高訊號 deterministic 子集，**不**取代 semgrep / snyk / GitHub code scanning 等深度 SAST，也不取代 08-security-agent.md 的 AI Security agent IDOR / Auth model 稽核；二者互補。**架構決策**：watcher / security 兩個 runner 共用相同 verdict aggregation / identity dataclass / filesystem helper 形狀，但本輪**不**抽出 `gate_runner_common` module（兩個 caller 還沒到 rule-of-three 門檻；待 P8 #4 QA runner 落地時再 extract，避免過早抽象）。`MIRROR OF watcher_gate_runner` comment 在 `security_gate_runner.py` 內標記未來 extract 候選。

- [x] 實作 qa checkpoint runner
  - 交付物：qa gate runtime
  - 驗收：QA gate 可阻擋未驗證輸出
  - 進度：done in `v0.22.0` (in-progress)；新增 `engine/qa_gate_runner.py` 提供純函式 `run_qa_gate()` 與四種機械檢查（`artifact_exists` / `artifact_non_empty` / `test_summary_patterns` / `coverage_threshold_patterns`），並在 `engine/step_runtime.py` 新增 `run-qa-gate` 子命令（額外 `--coverage-threshold` 預設 80.0）。Producer 沿用 P8 #1 watcher / #3 security 同一條 emit-then-self-validate skeleton 與 `_validate_gate_result_payload` helper。**Pattern bank**：3 條 test-summary 正則（jest `Tests: ... total` / pytest `===== ... in Ns =====` / mocha `N passing` + 可選 `N failing`）+ 3 條 coverage 正則（jest `All files | %` / generic `coverage: N%` / `Lines: N%`）；first-match-wins per artifact，跨多份 report 時 tests 欄位 sum / coverage 取 min（worst score wins），並把每個 dialect 累積到 `tests_dialects[]` / `coverage_dialects[]` 保留診斷 provenance。**Severity 策略**：test_failure → high/escalate（QA fail 可恢復，與 security secret leak 的 critical/halt 不同）；coverage_below_threshold → medium/warn；artifact_missing → blocked/high/halt；artifact_empty → medium/warn（QA 期待有內容，與 security 的 low 不同）；無 pattern 命中 → info finding（pass/low）。**Metrics 欄位**：`tests_total` / `tests_passed` / `tests_failed` / `coverage_percent` / `coverage_threshold` / `tests_dialects[]` / `coverage_dialects[]`。新增 `tests/scripts/test-qa-gate-runner.sh` 覆蓋 17 cases / 77 assertions（jest clean / jest fail / pytest clean / pytest fail / mocha clean / mocha fail / coverage above threshold / coverage below / fail+low coverage 同時觸發 / 無法解析 / empty / missing / 空 target / 預設 `--output` / 14 個 envelope 全部通過 round-trip validate-gate-result CLI / `--coverage-threshold` override / `--produced-by` 與 `--gate-subtype` 覆寫），並 wire 進 `smoke-per-stage.sh`。**邊界**：runner 只解析報告文字摘要，**不**執行 jest / pytest / Playwright / k6 / Lighthouse；深度行為驗證仍屬 07-qa-agent.md 主責。**架構決策**：本輪是**第三個** caller（rule-of-three trigger 已達），但**仍未**抽出 `gate_runner_common` module —— 留作獨立 refactor commit 讓本 commit 只含新增行為，`MIRROR OF watcher_gate_runner / security_gate_runner` comment 在 `qa_gate_runner.py` 內標記未來 extract 候選。

- [x] 實作 logger milestone runner
  - 交付物：logger archive runtime
  - 驗收：結案摘要可自動生成或派工
  - 進度：done in `v0.22.0` (in-progress)；新增 `engine/logger_gate_runner.py` 提供純函式 `run_logger_gate()` 與機械檢查（workflow-result existence / non-empty / JSON parse / workflow-result schema validation / result.md presence / archive-summary presence with mode-aware severity），並在 `engine/step_runtime.py` 新增 `run-logger-gate` 子命令。Producer 沿用 P8 refactor 後的 `gate_runner_common` (`emit_and_validate_or_exit` / `aggregate_verdict` / `check_artifact_*`)，**不**重複任何 sibling runner skeleton。**輸入形狀**：與 watcher / security / qa 不同——Logger 採 dedicated 路徑參數（`--workflow-result` 必填、`--result-md` 選填、`--archive-summary` 選填、`--mode milestone|final`），因為三個 artifact 各有命名角色（SSOT / human projection / final narrative），扁平 `--target-artifact` 列表會丟失 provenance；envelope 的 `target_artifacts` 仍由 runner 從提供路徑彙整。**Severity 策略**：missing/empty/parse/schema-invalid workflow-result → blocked/high/halt（governance 無法在缺 SSOT 上裁決）；result.md missing → warn/medium；archive-summary.md missing → mode-aware（`milestone` → warn/medium，`final` → blocked/high/halt）。**P7 對接**：runner 直接驗 `workflow-result.json` 對 `schemas/workflow-result.schema.yaml`，並從 validated payload 抽 `final_state` / `final_result` / `total_steps` / `completed/failed/skipped/blocked` / `failure_count` / `artifact_count` / `session_count` / `promote_candidate_count` / `workflow_result_validated` 等 metric 進 envelope，作為下游 archive / inspect 的單一機器讀寫源。新增 `tests/scripts/test-logger-gate-runner.sh` 覆蓋 13 cases / 58 assertions（happy path / milestone+missing archive=warn / final+missing archive=blocked / result.md missing=warn / wf missing|empty|parse|schema-invalid 四種 blocked / wf-only=pass / metrics 提取 / 預設 `--output` / 10 個 envelope 全通過 round-trip validate-gate-result CLI / `--produced-by` `--gate-subtype` `--task-id` 覆寫），並 wire 進 `smoke-per-stage.sh`。**邊界**：runner 僅做 archive readiness gate，**不**取代 99-logger-agent.md 的 AI Logger（Daily Devlog 敘事、ADR 評論、archive-summary.md 內文撰寫仍由 AI 主責）；二者互補：AI 寫 narrative，runner 驗 readiness 並 emit machine-readable verdict。
...
- [x] 實作 fail route handling
  - 交付物：failure routing runtime
  - 驗收：gate fail 能 halt 或 route back
  - 進度：done in `v0.22.0` (in-progress)；新增 `engine/gate_result_consumer.py` 提供純函式 `derive_decision(envelope) → GateConsumeDecision`，並在 `engine/step_runtime.py` 新增 `consume-gate-result` 子命令作為四個 producer envelope 的第一個 consumer。**Decision matrix（v1）**：`pass` / `warn` → `proceed`（不阻擋）；`fail` / `blocked` 缺 `fail_routing` → 保守 `halt` + `needs_supervisor=true`；`fail_routing.action=halt` → `halt` 帶 producer reason；`action=route_back` 且帶 `route_back_to_step` → `route_back`；缺 step → 降級保守 halt 並 `needs_supervisor`；`action=escalate` → `escalate` + `needs_supervisor=true`；`action=retry` → `retry_unsupported`（v1 與 P8 #8 rerun-failed-gate 控制面重疊，明確標 unsupported 等 #8 落地再升級）；`action=none` → `defer_to_workflow_yaml`（producer 主動放棄，由 caller 的 workflow YAML on_fail 接手）。**CLI shape**：`consume-gate-result <gate-result-path> [--workflow-log <path>] [--route-history <path>]`；exit 0=決策已輸出（caller 讀 stdout 自行行動）/ 41=schema invalid / 1=missing|parse error；stdout 採 `decision=...;result=...;risk=...;route_back_to_step=...;needs_supervisor=...;source_gate_id=...;source_step_id=...;reason=...` 單行格式給 shell wrapper grep。**Audit 雙軌**：`--workflow-log` 寫人類可讀單行（含 timestamp / gate / step / verdict / decision / reason）；`--route-history` 寫 JSONL 結構化記錄（每行一個 routing decision，可串成 routing trail）；雙軌可獨立啟用。新增 `tests/scripts/test-gate-result-consumer.sh` 覆蓋 16 cases / 46 assertions（pass / warn / fail+halt / fail+route_back / fail+route_back 缺 step 保守 halt / fail+retry → retry_unsupported / fail+escalate / fail+none → defer / blocked + 缺 fail_routing → 保守 halt / malformed JSON → exit 1 / schema invalid → exit 41 / missing artifact → exit 1 / `--workflow-log` 兩次 append 對齊行數 / `--route-history` JSONL records 都通過 schema spot-check / 真實 producer envelope 端對端 round-trip 兩例），並 wire 進 `smoke-per-stage.sh`。**邊界**：consumer 不直接 halt workflow，只 emit decision；caller（cap-workflow-exec.sh hook 或未來 Python orchestrator）讀 stdout 後自行決定是否繼續。本輪不接 `cap-workflow-exec.sh` runtime hook，避免功能新增與 wrapper 整合風險混在同一個 commit；hook 留下一輪獨立合進。**架構決策**：v1 範圍刻意限定 halt / route_back / escalate 三條主路徑；retry 預留 `retry_unsupported` decision tag（與 P8 #8 收斂後升級）；none 走 defer 不僭越 workflow YAML SSOT。

- [x] 支援 rerun failed gate
  - 交付物：CLI 或 runtime control
  - 驗收：可只重跑失敗 gate
  - 進度：done in `v0.22.0` (in-progress)；新增 `engine/gate_result_rerun.py` 提供純函式（`determine_rerun_eligibility` / `resolve_next_output_path` / `dispatch_rerun`）+ CLI helper，並在 `engine/step_runtime.py` 新增 `rerun-gate` 子命令作為四個 producer envelope 的第二個 consumer（與 P8 #6 fail-route consumer 並列）。**Eligibility**：`fail` / `blocked` 預設可重跑；`pass` / `warn` 需 `--force`，否則 stdout `rerun_action=skipped` + `skip_reason=verdict_<verdict>_no_force` 並 exit 0（明確拒絕，非錯誤）。**Dispatch by gate_type**：依 `envelope.gate_type` 路由到對應 runner，從 envelope 還原 `*GateInput` —— watcher / security 只需 `target_artifacts` + identity；qa 額外從 `metrics.coverage_threshold` 還原（保留原 threshold，不會默認改回 80）；logger 從 `target_artifacts` 位置慣例還原 dedicated paths（[0]=workflow_result、[1]=result_md、[2]=archive_summary，與 producer emit 順序對稱），並從 `metrics.mode` 還原 mode；未來新 gate_type 走 `UnsupportedGateTypeError` → CLI `reason=unsupported_gate_type` + exit 1。**版本化輸出**：預設 `<step_id>-<seq>.gate-result.json`，seq 從 2 起算（原始 `<step_id>.gate-result.json` 視為 seq=1）；scan 同目錄找最大現存 seq + 1，避免覆寫；`--output` 顯式覆寫此邏輯。**保留 audit**：原始 envelope **絕不**被覆寫，重跑後並列存放，operator `ls` 看得到完整時間線。**雙重 schema gate**：原始 envelope 載入後 schema-validate（fail → exit 41），新 envelope 落地後再次從磁碟讀回 validate（producer drift / persist truncation 都擋）。**雙軌 audit trail**：`--workflow-log` 寫人類單行（`gate-rerun gate=... step=... original_result=... action=executed new_result=... new_path=...`）；`--route-history` JSONL 結構化（`decision=rerun_executed` / `rerun_skipped` / `rerun_unsupported_gate_type` 三種 tag）與 P8 #6 同檔同格式，可被同一 stream-reader 消費。新增 `tests/scripts/test-rerun-gate.sh` 覆蓋 17 cases / 51 assertions（fail→executed、blocked→executed、pass→skipped、warn→skipped、pass+force→executed、版本化序號遞增、`--output` 覆寫、原始檔案保留、schema invalid=41、missing=1、malformed=1、unsupported_gate_type 邊界（schema enum 已封死所以 producer 端不可達，註明設計裁定）、QA `coverage_threshold` 從 metrics 還原（65.0 而非預設 80）、Logger `mode=final` 還原、Security 重新偵測 secret leak、`--workflow-log` / `--route-history` 雙寫、6 個重跑 envelope round-trip 通過 validate-gate-result CLI），並 wire 進 `smoke-per-stage.sh`。**Smoke baseline 從 40 升至 41 pass，27 fail 不動，零 regression。** 既有 7 個測試（4 producer + validate + schema + consumer）共 321 assertions 全綠。**邊界**：本輪 CLI 為 operator 操作介面；自動化 hook（fail-route handler 偵測 `retry_unsupported` decision 後自動 trigger rerun）留下一輪整合 commit。架構決策：retry / rerun 兩條控制面在 P8 #6 v1 故意分開（consumer emit `retry_unsupported` decision tag + rerun-gate CLI 由 operator/automation 觸發），讓「policy decision」與「機械執行」職責清晰；未來若要把 retry 升級為 first-class consumer action，只需在 `gate_result_consumer.derive_decision` 把 `retry_unsupported` 改為 `retry_request` 並讓 caller 根據它呼叫 rerun-gate。

- [x] enforce halt-on-risk
  - 交付物：risk policy enforcement
  - 驗收：高風險未通過 gate 不會繼續執行
  - 進度：done in `v0.22.0` (in-progress)；在 `engine/gate_result_consumer.py` 的 `derive_decision` 入口加上 P8 #7 halt-on-risk policy（**第 0 順位、優先於所有 fail_routing 推薦**）。新增模組常數 `HALT_ON_RISK_LEVELS = ("high", "critical")`，當 envelope 的 `risk_level` 落在此集合時，強制 `decision=halt` + `needs_supervisor=true`，不論 `result`（pass / warn / fail / blocked）或 `fail_routing.action`。**Reason 層保留全部 audit context**：`halt_on_risk policy: risk_level=<lvl> requires halt regardless of verdict=<v>`；如果 producer 提供 `fail_routing.action` 為非 halt 動作（route_back / escalate / retry / none），追加 `overrides producer fail_routing.action=<action>`；保留 producer 自己的 reason verbatim；對 fail/blocked 但缺 fail_routing 的情境保留 `fail_routing absent on fail/blocked verdict` 子字串以維持既有 triage 工具相容。**Notes 結構化 tag**：`halt_on_risk`（必有）+ `overrode_action:<action>`（被覆蓋時）+ `fail_routing_absent`（適用時），可被 `route-history.jsonl` JSONL 消費者用於 stream-level filter。**設計裁定**：高風險一律 halt 包括 `fail+route_back+critical` —— 避免高風險 finding 自動 route_back 到原 pipeline 形成自癒迴圈；`route_back_to_step` 在 halt-on-risk 路徑必為 `null`，確保不洩漏 producer 推薦的目標 step。**測試**：擴 `tests/scripts/test-gate-result-consumer.sh` 從 16 cases / 46 assertions 升至 26 cases / 78 assertions。既有 cases 3-9 將 `risk_level` 從 `high`/`critical` 降為 `medium`，讓既有 fail_routing.action 路徑（halt / route_back / retry / escalate / none / 保守 halt）能繼續被純粹測試（非被 halt-on-risk 蓋掉）；新增 cases 17-26 覆蓋 halt-on-risk 矩陣（pass+high → halt、pass+critical → halt、warn+critical → halt、warn+high → halt、fail+escalate+high → halt 並 supplant escalate、fail+route_back+critical → halt 且 route_back_to_step=none、fail+retry+high 確認非 retry_unsupported、fail+none+high 確認非 defer、blocked+critical+no fail_routing → halt 且保留 fail_routing absent tag、route-history JSONL 帶 halt_on_risk + overrode_action note）。**邊界**：純政策層改動，**不**新增 module / CLI / wrapper hook / 自動化 chain；既有 8 個 governance gate 測試（4 producer + validate + schema + consumer + rerun）共 404 assertions 全綠；smoke baseline 維持 41/27 零 regression。**P8 收尾**：本項完成後 P8 governance gates 8 個子項全部達成（contract → producer × 4 → 共用層 → consumer × 2 → 政策層）。
...
- [x] 支援 repo-local workflow source roots
  - 交付物：workflow resolver
  - 驗收：project workflow 可覆蓋或擴充 builtin workflow
  - 進度：done per `docs/cap/P9-SOURCE-RESOLVER-DESIGN.md` §4.1 / §4.2 / §4.5。`engine/workflow_loader.py:WorkflowLoader.__init__` 增 keyword-only `project_root` / `cap_home`（precedence：explicit kwarg → `CAP_PROJECT_ROOT` / `CAP_HOME` env → `Path.cwd()` / `Path.home()/.cap` fallback，**不 shell-out 呼 `cap-paths.sh`**）；新增 `_resolve_workflow_path(ref) → (Path, source_layer)` 與 `_infer_source_layer(path)` 純函式，先試絕對 / cwd-relative 路徑（缺 raise；存在則推 layer），否則依序掃 `project` → `shared` → `builtin`，全部 miss 拋 `FileNotFoundError` 並列出三個搜過的 dir。`load_workflow` 改寫成呼叫 `_resolve_workflow_path` 後把 `source_layer` 傳給 `normalize_workflow_data`，後者把 `_source_layer` 寫進 normalized dict（既有 `_source_path` 維持不變）。`engine/workflow_cli.py:cmd_resolve_ref` signature 從 `(workflows_dir, raw_ref)` 改為 `(raw_ref, workflows_dir=None)`，parser 把 `workflows_dir` 從 positional 降為 deprecated `--workflows-dir` flag（傳入時 stderr warn 但沿用單層 scan）；缺旗標時走 `WorkflowLoader` 三層 layered scan，每層先試 filename + 4 種 suffix 變體，再 fallback 到 `workflow_id` / `short_id` / stem / 全名比對（保 `cap workflow run <workflow_id>` 既有語意）。`scripts/cap-workflow.sh:resolve_workflow_ref` 移除 `${WORKFLOWS_DIR}/${raw_ref}{,.yaml}` fast-path（避免 builtin fast-path 偷走 project override），保留絕對路徑 short-circuit，其餘直接 forward 給 `${PYTHON_BIN} ${CLI_PY} resolve-ref ${raw_ref}`。`tests/scripts/test-workflow-source-resolver.sh` 8 cases / 14 assertions（同 id override / 新 id extend / shared hit / 三層 miss + error 訊息列三 dir / absolute under project / absolute under builtin / absolute outside three layers→explicit / cap-protocols self-mode 雙路徑共存 / bash 委派與 CLI 結果一致）；macOS `/var → /private/var` symlink 在 sandbox 用 `cd "${tmp}" && pwd -P` canonicalize 處理。**未動範圍**（留給 P9 #3-#5）：`RuntimeBinder` / skill registry merge / `binding-report.schema.yaml` / allowed source roots enforcement。回歸：`test-compiled-workflow-normalization` (8) + `test-cap-config-namespace-readers` (27) + `test-result-report-builder` (91) + `test-cap-workflow-inspect` (46) 全綠零 regression。

- [x] 支援 repo-local skill registry
  - 交付物：skill resolver
  - 驗收：project skill 可被 binding pipeline 使用
  - 進度：done per `docs/cap/P9-SOURCE-RESOLVER-DESIGN.md` §5。`engine/runtime_binder.py:RuntimeBinder.__init__` 加 keyword-only `project_root` / `cap_home`（precedence：explicit kwarg > env (`CAP_PROJECT_ROOT` / `CAP_HOME`) > **explicit `base_dir` 視為整個宇宙** > cwd / `~/.cap` fallback；最後一條 backward-compat layer 是為了不破壞既有「`RuntimeBinder(base_dir=sandbox)`」的測試契約）；同層 root 透過 kwarg pass-through 給 `WorkflowLoader`，並且 `WorkflowLoader.__init__` 也補同樣 backward-compat layer 對齊。`load_skill_registry(registry_ref=None)` 改為 dispatcher：傳 `registry_ref` 走新 `_load_single_registry`（保 pre-P9 #3 單檔行為，包含 legacy `agents` envelope 自動 adapt），未傳走新 `_load_layered_skill_registry`，依 `project` → `shared` → `builtin` 收集每層後合併（priority project > shared > builtin）；三層皆空 fall through 到既有 `_load_legacy_registry_adapter` 維持 backward compat。新增 helper `_resolve_layer_registry(layer_name, layer_dir)` 處理單層輸入：先試 flat `<dir>/skills.{yaml,yml,json}`、否則掃 `<dir>/skills/*.{yaml,yml,json}`；builtin layer 額外保留 `<base_dir>/.cap.skills.yaml` 舊散檔 fallback。`_merge_skill_layers` 以 `skill_id` 為 key 做 first-encountered-wins dedupe，並用 `_deep_merge` 對 `binding_defaults` 做 key-wise 遞迴合併（high layer 任何 key 都贏，含 nested dict；list 整段替換不 element-wise merge）。每個 skill entry 內部標 `_source_layer` 與 `_source_path` 供 P9 #4 binding-report producer 消費；merge 結果頂層加 `_source_paths[]` 列出所有貢獻檔。Self-mode（`project_root.samefile(builtin_dir)`）跳過 project layer 不雙載（與 P9 #2 同精神）。新增 `tests/scripts/test-skill-registry-resolver.sh` 8 cases / 22 assertions（project override builtin、project extend、shared-only、project beats shared、explicit `registry_ref` short-circuit、self-mode 不雙載、binding_defaults 深合併 5 子斷言、`_source_layer` / `_source_path` tags）。**未動範圍**（留給 P9 #4-#5）：`binding-report.schema.yaml` source metadata、allowed_source_roots enforcement extension。回歸：cap-config-namespace-readers (27) + workflow-policy-gates (19) + workflow-source-resolver (14) + compiled-workflow-normalization (8) + 4 個 P7 suites (175) 全綠零 regression。

- [x] 明確區分 builtin / project / shared skill source
  - 交付物：source metadata
  - 驗收：binding report 顯示每個 skill 的來源
  - 進度：done per `docs/cap/P9-SOURCE-RESOLVER-DESIGN.md` §6。`schemas/binding-report.schema.yaml` 加 3 個 optional 欄位（**不破 backward compat**）：頂層 `workflow_source: {source_layer, source_path}`（layer enum：`project / shared / builtin / explicit`，nullable for synthesized inline plans）；頂層 `effective_allowed_roots: []`（**P9 #4 placeholder，實際值由 P9 #5 計算**）；每 step entry `skill_source: {source_layer, source_path}`（layer enum 多 `fallback`，nullable for unresolved/blocked branches）。`engine/workflow_loader.py:build_semantic_plan_from_workflow` thread `_source_layer` 進 semantic plan（key 名 `source_layer`），讓 binder 拿得到。`engine/runtime_binder.py:bind_semantic_plan` 結尾組裝 binding 時：根據 `semantic_plan.source_layer` + `source_path` 寫頂層 `workflow_source`（兩個都缺視為 inline，寫 null），寫 `effective_allowed_roots: []` placeholder；新 helper `_skill_source_metadata(skill, *, fallback_when_missing=False)` 把 P9 #3 的 `_source_layer` / `_source_path` 內部 tag 投影成 binding-report shape，無 layer tag（adapter-synthesised 或 builtin-shell）回 `{"source_layer": "fallback", "source_path": null}`，無 skill 回 `None`。每個 `step_reports.append({...})` 4 處 site（bootstrap-blocked / capability-blocked / shell / 主 resolution）都 inject `skill_source`，主 resolution 用新 local `chosen_skill` 追蹤實際選定的 skill dict（resolved → selected / fallback_available → fallback / incompatible → broken / unresolved → None）。新增 `tests/scripts/test-binding-source-metadata.sh` 6 cases / 17 assertions（builtin-only binding 含 AI/shell/unresolved 三種 skill_source 形狀 + workflow_source + effective_allowed_roots + schema 驗證；project-override 覆蓋 builtin 的 skill_source.source_layer=project；synthesised inline plan workflow_source=null + schema 仍過）。**踩坑修正**：(1) `json_field` helper 原本用 `printf | python - <<EOF` 同時佔 stdin，heredoc 蓋掉 pipe 導致 eval 讀到 Python 源碼而非 JSON；改用 argv 傳 JSON。(2) Sandbox 缺 git repo 與 `.cap.project.yaml`，ProjectContextLoader project_id 解析需 export `CAP_PROJECT_ID_OVERRIDE`，並 export `CAP_HOME=${SANDBOX}/cap_home` 把 ledger 落 sandbox 內避免 cross-run 衝突。(3) Bootstrap mode（無 constitution）會把每個 step 路由到 `blocked_by_constitution`，需 plant `.cap.constitution.yaml`（legacy 路徑，因為 `DEFAULT_PROJECT_CONSTITUTION` 是 legacy）讓 `_bootstrap=False`。**未動範圍**（P9 #5）：`effective_allowed_roots` 的真實計算與兩個 hook（`_assert_workflow_source_allowed` 升級 + 新 `_assert_skill_source_allowed`）的 enforcement。回歸：12 個 suite 共 307 assertions 全綠。

- [x] 套用 Project Constitution allowed source roots
  - 交付物：source root enforcement
  - 驗收：未允許來源不會被載入
  - 進度：done per `docs/cap/P9-SOURCE-RESOLVER-DESIGN.md` §7。`engine/runtime_binder.py` 新增 `SourcePolicyError` 共同基底（`WorkflowSourcePolicyError` 與新 `SkillSourcePolicyError` 都繼承之，`stage` / `errors` 構造契約不變，既有 `except WorkflowSourcePolicyError` callers 不需改）。新 `_compute_effective_allowed_roots(project_context) -> list[str]` 一次計算 effective set：`enforce_allowed_source_roots=False` → 回 `[]`；否則依序 union user-declared `constitution.workflow_policy.allowed_source_roots`（讀順序保留以利 binding report 顯示）→ 隱式 project layer (`<project_root>/.cap/{workflows,skills,skills.yaml,skills.yml,skills.json}`) → 隱式 builtin layer (`<base_dir>/{schemas/workflows,.cap/skills,.cap/skills.yaml,.cap/skills.yml,.cap/skills.json}`) + legacy fallback (`<base_dir>/.cap.skills.{yaml,yml,json}`)；deduped 保 priority 順序。**修正 memo §3.1.2 的 typo**：memo 列 `.cap/skills.json` 但 canonical 是 `.cap/skills.yaml`（per `DEFAULT_REGISTRY_PATH_NAMESPACED`），實作三 extensions 全收避免既有 `enforce=true` 專案被自己 builtin registry 擋死。新 `_path_is_under_any_root(source_path, allowed_roots)` 純函式 helper 集中 `Path.resolve()` + `relative_to` / parents 比對，兩個 enforcement hook 共用。`_assert_workflow_source_allowed` 升級成走 `_compute_effective_allowed_roots`：原邏輯只看 `workflow_policy.allowed_source_roots` 直接列表，現在 implicit + user-declared 一起；synthesised 路徑（`<...>` 開頭）仍 short-circuit。新 `_assert_skill_source_allowed(skill_source, *, step_id, effective_allowed_roots)`：`effective_allowed_roots=[]` 跳過、`skill_source=None` 跳過（unresolved/blocked 無 skill）、`skill_source.source_path=None` 跳過（fallback 合成或 builtin-shell 無真實檔），其他情況比對 effective set，違反 raise `SkillSourcePolicyError(stage="skill_source_policy", ...)`，**halt 整個 binding 不降級**（memo §7.4：governance redline 優先於可用性，降級會把違規隱藏在 binding report 的 fallback 標籤）。`bind_semantic_plan` wire-in：開頭 compute 一次 `effective_allowed_roots`，呼 workflow gate（與既有相同點）；4 個 step append site 都加 `_assert_skill_source_allowed` 呼叫保持「每 step 都 gate」一致性（bootstrap-blocked / capability-blocked / shell-executor 是結構性 no-op；主 resolution 才會真正 raise）；最後組裝 binding 把 `effective_allowed_roots` 從 P9 #4 的 placeholder `[]` 換成真實 snapshot。`engine/workflow_cli.py:cmd_compile_json` 補 `SkillSourcePolicyError` handler，吐 deterministic JSON `{"ok": false, "error": "skill_source_policy_error", "stage": ..., "errors": [...]}` exit 1，與既有 `workflow_source_policy_error` 對稱。新增 `tests/scripts/test-source-roots-enforcement.sh` 7 cases / 20 assertions（`enforce=false` 兩 gate 皆 skip + `effective_allowed_roots=[]` / `enforce=true` 隱式 defaults 蓋住 canonical project + builtin 路徑 / shared 不在 user list 觸發 `SkillSourcePolicyError` halt / 顯式宣告 shared 後 pass / workflow gate 比 skill gate 先 fire / 異常 hierarchy `WorkflowSourcePolicyError + SkillSourcePolicyError` 都 inherit `SourcePolicyError` 且 `stage` / `errors` 契約 carry-through / `effective_allowed_roots` 順序 user-declared 在前）。**未動範圍**：P10 promote candidates。回歸：13 個 suite 共 327 assertions 全綠零 regression。
...
- [x] **P10 #2**：promote candidate producer
  ...
  - 進度：split 為 #2.1 schema migration + #2.2 producer 兩個 commit。**#2.1**（commit `e8054a5`）— `schemas/workflow-result.schema.yaml.promote_candidates[].items` 從 P0 carried 的 loose `(artifact_name, path, target_repo_path, reason)` 形改為 §5.2 嚴格契約：required `[source_path, target_path, artifact_type, reason]` + optional `validation_schema` / `source_layer` / `source_revision`；`artifact_type` 嚴格 enum `[project_constitution, compiled_workflow]`，禁 binding_report / run-only artifacts；`tests/scripts/test-workflow-result-schema.sh` positive fixture 對齊新 shape，新增 Negative 9（缺 `artifact_type`）+ Negative 10（`artifact_type=binding_report` 違 enum），10 → 12 cases。**#2.2**（本 commit）— 新增 `engine/promote_candidate_producer.py:produce_candidates(run_result, *, project_root=None, cap_home=None)`：純 read-only path-existence check；project_constitution candidate 在 `task_id` 非空且 `<cap_home>/projects/<id>/constitutions/<task_id>/constitution.{yaml,yml,json}` 存在時 emit；compiled_workflow candidate 在 `final_state=="completed"` 且 `<cap_home>/projects/<id>/compiled-workflows/<workflow_id>/` 有 `.json/.yaml/.yml` 檔時 emit（取 mtime 最新，filename 為 tiebreaker）；`final_state` 不為 completed（failed / blocked / cancelled / running）一律不 emit compiled_workflow（policy §5.3 安全紅線：壞 run 的 compiled workflow 不該凍進 repo）；`project_id="unknown"` 或缺 → return `[]`；missing source on disk → silent no-emit 不 raise（policy §5.3）；不讀任何檔案內容、不讀 schema、不讀 P3 envelope（policy §5.4）。`build_workflow_result` 加 keyword-only `project_root` arg（precedence: kwarg > `CAP_PROJECT_ROOT` env > `cwd`，與 P9 `WorkflowLoader` / `RuntimeBinder` 對齊），新 helper `_produce_promote_candidates` thin wrapper 把 producer import 推遲且 swallow 任何 exception 退回 `[]`（degrade silently — promote candidates 是 advisory，不該擋 run）。`scripts/cap-result-emit.sh` 加 `project_root_arg` 偵測（`CAP_PROJECT_ROOT` env 優先，否則 `cap-paths.sh get project_root` shell-out）並把 6 個 positional args（含新 `project_root`）一齊塞給 inline Python invocation。新增 `tests/scripts/test-promote-candidate-producer.sh` 8 cases / 24 assertions（project_constitution emit + 5 個 fields 形狀 / compiled_workflow emit + target_path / final_state 4 種非 completed 都 no-emit / task_id missing / project_id="unknown" 或缺 / 兩種 emit 一起時 project 在前 / 通過 schema validation / `build_workflow_result` wire-in 把 producer 結果寫進 `result["promote_candidates"]`）。回歸：14 個 suite 共 353 assertions 全綠零 regression。

- [x] **P10 #3**：`cap promote inspect <artifact_id>`
  - 交付物：CLI subcommand
  - 驗收：顯示 artifact 是否可 promote、source / target / 風險、會覆寫什麼、需要哪些 validation；支援 `--json`
  - 對齊 `policies/runtime-promote.md` §7.1
  - 進度：done。**Producer refactor**：`engine/promote_candidate_producer.py` 把舊 `_detect_constitution_candidate` / `_detect_compiled_workflow_candidate` 重構成 typed public helpers `detect_constitution_candidate_for_task(task_id, *, project_storage, project_root, source_layer)` 與 `detect_compiled_workflow_candidate_for(workflow_id, final_state, *, project_storage, project_root, source_layer, require_completed=True)`，原 internal helpers 變成 thin wrappers 給 `produce_candidates` 用；resolver 直接 import typed helpers，policy §5.3 final_state 安全紅線維持但 inspect 路徑用 `require_completed=False` 仍能描述 on-disk snapshot（gate 在 apply 時 fire 而非 inspect）。**Resolver**：新模組 `engine/promote_resolver.py` 引入 `ResolvedPromote` frozen dataclass（`candidate / target_exists / conflict_kind / backup_path / backup_required / validation_schema / smoke_plan`）與 `resolve_promote(artifact_id, *, project_id, project_root, cap_home)`：依序試 task_id（constitution）→ workflow_id（compiled workflow），first match 勝、兩者 miss → `None`。`conflict_kind` 三 enum：`no_target` / `identical` / `diff`（用 `filecmp.cmp(shallow=False)` 做 byte-equal 對比，I/O error 退回 safe diff branch）；`backup_path` 用 `<target>.bak.<ISO>` template（不 generate 真實 timestamp，避免 inspect 與 apply 的 timestamp 不一致誤導使用者）；`smoke_plan` 描述 schema_validate（policy §6.1 always）+ compile_bind_smoke（policy §6.3 opt-in via `--smoke`）。`make_template_backup_path(target, *, timestamp=None)` helper 給 #4 apply 重用同樣命名規範。Project_id 解析優先 `CAP_PROJECT_ID_OVERRIDE` env，再 `<project_root>/.cap/project.yaml` namespaced + legacy fallback；resolver 對 `project_id="unknown"` 或缺直接 return `None`，inspect CLI 渲染為 deterministic `promote_artifact_not_found` error。**CLI Python entry**：新模組 `engine/promote_cli.py:cmd_inspect` + `main()` argparse，`<artifact_id>` 加 `--json` / `--project-root` / `--cap-home` / `--project-id`；text mode 4 sections（Header / Source / Target / Validation），JSON mode 吐 `ResolvedPromote` dataclass `asdict` + `ok=True`；miss 時 exit 1 + `{"ok": false, "error": "promote_artifact_not_found", "artifact_id": ..., "message": ...}`。`scripts/cap-promote.sh` 加 `inspect` subcommand dispatch（forward `"$@"` 給 Python argparse），既有 `list` 與 generic `<src> <dst>` 兩條 escape hatch 維持（policy §7.4 / §8.4）。**測試**：`tests/scripts/test-cap-promote-inspect.sh` 10 cases / 32 assertions（task_id no_target / identical / diff + backup template / workflow_id smoke plan / final_state-agnostic at inspect / unknown artifact 走 not_found exit 1 / text 4 sections / project_id="unknown" / bash dispatch 與 Python 結果一致 / validation_schema 在 text + json 都出現）。回歸：15 個 suite 共 385 assertions 全綠零 regression。**未動範圍**（#4-#8）：apply / write / backup / validation / rollback / project-constitution / workflow CLI / docs / 整合測試。

- [x] **P10 #4**：`cap promote project-constitution <task_id>` 收斂
  ...
  - 進度：done。**Apply primitives (`engine/promote_apply.py`)**：可重用 apply 框架，#5 workflow promote 共用同一路徑只差 `expected_artifact_type`。`ApplyResult` frozen dataclass + 11 個 action enum (`dry_run_no_target` / `dry_run_identical` / `dry_run_conflict` / `dry_run_force_overwrite` / `applied_fresh` / `applied_identical_skip` / `applied_force_with_backup` / `halted_conflict` / `halted_type_mismatch` / `halted_backup_failed` / `halted_write_failed` / `validation_failed_rolled_back` / `validation_failed_rollback_failed`)，`ok` property = `error is None and action.startswith(("applied_","dry_run_"))`。`apply_promote(resolved, *, expected_artifact_type, dry_run, force, artifact_id, cap_root)` 走完整流程：type mismatch → halt；dry_run → 描述 4 種 sub-action 不寫；identical → short-circuit `applied_identical_skip`；diff 無 force → halt with `promote_conflict_halt`；diff + force → backup（用 P10 #3 `make_template_backup_path` 補真實 ISO timestamp）→ 寫 target → schema validate → 失敗 rollback；no_target → 直接寫 → validate → 失敗 rollback（target 不存在則 rollback 用 `unlink(missing_ok=True)`）。`_validate_target_via_step_runtime(target, validation_schema, *, cap_root)` 替代原本 subprocess shell-out — 改 inline YAML/JSON 載入 + `jsonschema.Draft202012Validator`（缺 jsonschema 時 fallback `engine.step_runtime.validate_jsonschema_fallback`），canonical `{"ok": bool, "errors": list[str]}` 結構；原 subprocess 路徑只能讀 JSON target，遇到 YAML constitution 會 parse fail，refactor 後支援兩種格式且不需 subprocess overhead。`_rollback_target(target, backup_path, target_existed_before)` 負責 rollback：原本不存在 → `unlink`；原本存在 + backup → `shutil.copy2` 從 backup 還原；原本存在但無 backup（理論上不該發生）→ 回 fail 讓 caller surface。Backup 永不自動清（policy §4.3）。**CLI (`engine/promote_cli.py:cmd_project_constitution`)**：`<task_id>` + `[--apply]` (default `--dry-run`) + `[--force]` + `[--json]` + `--project-root` / `--cap-home` / `--project-id`；resolver 不回 candidate 或 `artifact_type != project_constitution` 時走 `promote_artifact_not_found` exit 1；`apply_promote` 結果經 `_emit_apply_result` 渲染 — JSON 模式統一形狀（`ok` / `action` / `artifact_id` / `artifact_type` / `source_path` / `target_path` / `target_existed_before` / `backup_path` / `validation` / `error` / `detail`），text 模式 5 sections（Header / Source / Target / Backup / Validation）。**Bash dispatch (`scripts/cap-promote.sh`)**：加 `project-constitution` subcommand forward 給 Python CLI；既有 `inspect` / `list` / generic copy 維持。**測試**：`tests/scripts/test-cap-promote-project-constitution.sh` 13 cases / 37 assertions（4 種 dry-run / `applied_fresh` + validation passes / `applied_identical_skip` 不重跑 validation / `halted_conflict` 不寫 target / `applied_force_with_backup` 真 timestamp + backup 檔落地 + target 與 source byte-equal / 無效 source 觸發 `validation_failed_rolled_back` 並 rollback 到 pre-apply state / type mismatch / unknown task_id / legacy `.cap.constitution.yaml` 全程從未被寫 — policy §3.1 紅線 / bash dispatcher 與 Python CLI 結果一致 / valid constitution fixture 對齊 9 個 schema required fields）。**未動範圍**（#5 / #7 / #8）：workflow promote、user docs、跨 sub-item 整合測試。回歸：16 個 suite 共 422 assertions 全綠零 regression。
