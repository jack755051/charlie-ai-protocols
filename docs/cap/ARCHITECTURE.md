...
| `cap claude [ARGS...]` | `cap-entry.sh:claude` → [`scripts/cap-session.sh`](../../scripts/cap-session.sh) | 僅在可解析 project identity 時觸發；非 CAP 目錄退回原生 Claude |
| `cap codex [ARGS...]` | `cap-entry.sh:codex` → `cap-session.sh` | 僅在可解析 project identity 時觸發；非 CAP 目錄退回原生 Codex |
| `cap workflow run --cli claude` | `cap-workflow-exec.sh` 派工 | 是（workflow runtime 內 step 才觸發） |

**核心規則**：global `~/.zshrc` 只應註冊 `cap()` 一個 shell function；裸 `claude` / `codex` 必須維持原生 provider 行為。CAP 自動 trace recording 透過顯式 `cap claude` / `cap codex` 入口取得；若目前目錄沒有 git repo、`.cap.project.yaml`、`CAP_PROJECT_ID_OVERRIDE` 或 legacy fallback 設定，顯式入口也會退回原生 provider。
...
- **`project_id` resolver 副作用**：CAP-wrapped `claude` 在 `~` 或任何非 CAP 目錄呼叫時，不應觸發 `cap-session.sh` → `cap-paths.sh` strict resolver；v0.22.x P0b 後會直接退回原生 provider，避免產生意外的 `~/.cap/projects/<dir>/...` 目錄結構。
...
| `cap claude` 在 `~` 執行 | 退回原生 Claude，不寫 trace | 若裸命令被 opt-in wrapper 攔截，同樣退回原生 Claude |
...
- `scripts/cap-session.sh:has_cap_project_context` — 在 trace 前判斷是否可解析 project identity；不可解析時 exec 原生 provider
...
## 🗂 Config vs Runtime Storage Boundary

> 落點：v0.22.x P0c CAP Config Namespace Migration batch 1（read-only compat layer）。劃清「專案配置（user-owned，repo-tracked）」與「執行期 storage（CAP-owned，gitignored）」的界線；目的是讓使用者不必把 6 個 `.cap.*` 散檔扔在 repo 根目錄，也讓未來 promote / multi-repo workflow 能精準辨識「config 跟著 repo 走」vs「runtime 跟著機器走」。

### 兩個物理隔離區

| 角色 | 路徑 | 擁有者 | Git tracked | 內容 |
|---|---|---|---|---|
| **Project Config Namespace** | `<repo_root>/.cap/`（新）<br>`<repo_root>/.cap.*`（legacy） | user / repo maintainer | ✅ 是 | `project.yaml` / `constitution.yaml` / `skills.yaml` / `agents.json` 等 repo 級設定 |
| **Runtime Storage** | `~/.cap/projects/<project_id>/` | CAP runtime | ❌ 否 | snapshots / bindings / handoffs / sessions / reports / traces |

關鍵分界：**`<repo_root>/.cap/` 永遠不應包含 runtime 產物**。`cap workflow run` 寫入的所有 trace、handoff、snapshot 都落 `~/.cap/projects/<id>/`，跟著機器走；repo `.cap/` 只放使用者願意 commit 進 git 的配置。

### Config Namespace（batch 1 read-only compat）

| 檔案 | 新路徑（建議） | Legacy 路徑（仍支援） | Resolver 行為 |
|---|---|---|---|
| Project ID | `<repo>/.cap/project.yaml` | `<repo>/.cap.project.yaml` | 新路徑優先；都不存在 → strict halt（exit 52）或 git_basename fallback |
| Project Constitution | `<repo>/.cap/constitution.yaml` | `<repo>/.cap.constitution.yaml` | 由 `.cap/project.yaml:constitution_file` 顯式指定（batch 1 不改 default） |
| Skill Registry | `<repo>/.cap/skills.yaml` | `<repo>/.cap.skills.yaml` | 同上（batch 2 才改 default） |
| Agent Registry | `<repo>/.cap/agents.json` | `<repo>/.cap.agents.json` | 同上（batch 2 才改 default） |

**Batch 1 範圍只動 project_id resolver**：4 個 reader（`scripts/cap-paths.sh:read_project_id_from_config` / `engine/project_constitution_runner.py:resolve_project_id` / `engine/project_context_loader.py:ProjectContextLoader.load` / `engine/step_runtime.py:_project_id_from_config`）支援雙路徑讀取，新路徑優先。其他配置檔（constitution / skills / agents）的 default 切換留給 batch 2，使用者可自行在 `.cap/project.yaml` 用 `constitution_file: .cap/constitution.yaml` 顯式指定來預覽 batch 2 行為。

### 為什麼要做 namespace migration

- **Repo root 雜亂**：6 個 dotfile 散落在 root（`.cap.project.yaml` / `.cap.constitution.yaml` / `.cap.skills.yaml` / `.cap.agents.json` / 等），新增配置時更難辨識歸屬。
- **Promote / multi-repo 治理需要**：未來 multi-repo workflow 會跨 repo 比對 config 一致性；單一目錄 `.cap/` 比 6 個分散 dotfile 容易遞迴比對。
- **跟 runtime storage 對稱**：runtime side 已是 `~/.cap/projects/<id>/<sub>/` 兩層命名空間；config side 遲早要對齊成 `<repo>/.cap/<config>` 對稱結構。
- **Provider Isolation 修完後的延伸**：P0b 把全域包裹拔掉、P0c 把 repo 配置歸位，兩件加起來才完整解決「CAP 不該污染 user 全域空間 + 不該霸佔 repo root」的設計缺陷。
...
`~/.cap/projects/<project_id>/` 的 layout（constitutions / compiled-workflows / bindings / reports / traces / sessions）**不**屬於 namespace migration 改動範圍；那是 runtime side 的 SSOT，由 P1 storage health 與 P3 orchestration storage boundary memos 治理。本節只關心 **repo-tracked config 入口**從散檔 dotfile 收斂到單一 `.cap/` 目錄。

### 實作 SSOT

- Shell：`scripts/cap-paths.sh:read_project_id_from_config` — 新→legacy 雙路徑讀，新路徑優先
