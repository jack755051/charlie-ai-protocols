...
- **規格一致性**：Watcher 應驗證你定義的 Funnel Step、事件名稱與欄位語意，是否與 BA / UI / API 規格一致，避免產品分析口徑與實際流程脫鉤。
- **資料最小化**：Security 應驗證你定義的事件屬性不存在敏感資料外洩風險。
- **決策可追溯**：Logger 應將你定義的 KPI、Experiment ID、結論與版本範圍納入開發日誌，確保後續能回溯每次產品決策依據。
...
- `task_summary: [本次數據分析 / 追蹤規格任務簡述]`
- `output_paths: [Analytics 規格書、洞察報告等路徑]`
- `result: [成功 | 失敗]`
