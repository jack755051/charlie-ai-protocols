...
- [x] **P3 #5-b compile/bind transition (current branch)**：在 `engine/task_scoped_compiler.py` 新增 `TaskScopedWorkflowCompiler.compile_task_from_envelope(envelope, registry_ref=None)` 入口，與既有 `compile_task(source_request, registry_ref=None)` 並行；envelope-driven path 入口跑 `engine.supervisor_envelope.validate_envelope` + `check_envelope_drift` 雙重 gate，任一失敗 raise 新 exception class `CompileFromEnvelopeError`（schema fail / drift detected 兩條訊息對應 P3 #4 executor 的相同失敗類別）。Hybrid merge 策略：先跑 deterministic `build_task_constitution(envelope.source_request)` 拿 internal hints baseline（risk_profile / unresolved_policy / inferred_context / output_expectations），再以 envelope.task_constitution 的欄位 dict-spread override（envelope 主導 task_id / goal / goal_stage / success_criteria / non_goals / execution_plan / project_id / source_request）。Step 3-5（`build_candidate_workflow` / binder / `build_unresolved_policy` / `apply_unresolved_policy` / `build_bound_execution_phases_from_workflow`）共享 backbone，binder 簽章 0 改動。Output dict 在 legacy 7 key 上多一個 `compile_hints_applied` 純記錄（envelope.compile_hints 完整 round-trip，registry_preference / fallback_policy / preferred_cli / skip_optional_unresolved / attach_inputs / notes）；本 commit 不把 hints 翻成 binder 行為改動，只做 traceability，行為改動留 binder 改造另開 commit。新增 `tests/scripts/test-compile-task-from-envelope.sh` 6 cases / 22 assertions（happy + envelope-wins merge + schema fail raise + drift raise + legacy compile_task 不受影響 + 完整 hints round-trip + 空 hints 仍 surface），接入 smoke-per-stage 為 case 33。**範圍嚴格守住**：不改 `compile_task`、不改 `runtime_binder`、不改既有 workflow YAML、不做 failure routing dispatch；P3 #5-c 才動 workflow YAML wiring。
- [x] **P3 #5-c workflow YAML wire (current branch)**：新增 dedicated workflow `schemas/workflows/supervisor-orchestration.yaml`（單一 step `validate_supervisor_envelope` 引用 P3 #4 落地的 `supervisor_envelope_validation` capability + shell executor `scripts/workflows/validate-supervisor-envelope.sh`）；governance 採 `final_only` watcher / `milestone_log` logger 對齊 P0 #4 envelope.governance 預設。`.cap.constitution.yaml` `allowed_capabilities` 補 `supervisor_envelope_validation` 條目（P3 #4 commit 漏補的 binding 必要拼圖；缺此條 binding 會永遠 `blocked_by_constitution`）。`scripts/workflows/smoke-per-stage.sh` 加 `run_bind "supervisor-orchestration"` 為 binding case 4，跑 `cap workflow bind` 確認 `binding_status: ready` + `required_unresolved=0`，與既有 project-spec / project-implementation / project-qa pipeline 三條 binding check 同模式。**範圍嚴格守住**：不為 P3 #5-a snapshot writer / P3 #5-b `compile_task_from_envelope` 補 capability + shell wrapper（後續 cycle 工作）；不接到 `project-constitution.yaml` 或 per-stage pipelines；不做 P3 #6 failure routing dispatch；不打 release tag。完成 P3 #5-c 即收斂整段 P3 #5。
- [x] **P3 #6 failure routing resolver + xref (current branch)**：在 `engine/supervisor_envelope.py` 新增兩個 pure helper：`check_failure_routing_xrefs(envelope)` 偵測三類 dangling step_id（`default_route_back_to_step` / `overrides[].step_id` / `overrides[].route_back_to_step`）→ 回新 `XrefReport(ok, mismatches)` dataclass；`resolve_failure_routing(envelope)` 依 capability_graph node 順序輸出 per-step 對應表，每筆含 `step_id` / `on_fail` / `route_back_to_step` / `max_retries` / `source` (default|override)。CLI 加兩個 subcommand `xref` / `resolve` 與既有 extract / validate / drift 並列。`engine/task_scoped_compiler.py:compile_task_from_envelope` 入口在 schema validate + drift check 之後加 xref gate（dangling step_id → raise `CompileFromEnvelopeError` 訊息含 "xref" 區別三類失敗），output dict 從 8 key 升到 9 key 多 `failure_routing_resolved` 鍵。`tests/scripts/test-supervisor-envelope-helper.sh` 從 15 cases 擴到 24 cases / 61 assertions（+xref 5 cases / resolve 4 cases）；`tests/scripts/test-compile-task-from-envelope.sh` 從 6 cases 擴到 8 cases / 33 assertions（+routing resolved output / xref dangling raises），smoke-per-stage 不動（既有 case 30 / 33 fixture 內部擴張）。**範圍嚴格守住**：純 Python helpers；不動 `validate-supervisor-envelope.sh` shell executor、不動 `emit-handoff-ticket.sh` ticket emitter、不動 workflow YAML；不為 P3 #5-a snapshot writer 補 capability wrapper；不打 release tag。Runtime dispatcher（step 失敗時實際 halt / retry / route_back / escalate 行為）屬 P5 AgentSessionRunner 範疇，非本 commit。
- [x] **P3 #7 ARCHITECTURE / cap-entry visibility (current branch)**：`docs/cap/ARCHITECTURE.md` 新增三個獨立章節：(1)「🎼 Supervisor Orchestration」含 P3 #1-#6 已落地 commit 表 / envelope flow ASCII 圖 / 5 條 module map（`engine/supervisor_envelope.py` / `engine/orchestration_snapshot.py` / `engine/task_scoped_compiler.py` / `scripts/workflows/validate-supervisor-envelope.sh` / `schemas/workflows/supervisor-orchestration.yaml`） / 已知未接通清單（明確標記 runtime dispatcher 尚未接通、envelope→ticket 鏈路尚未連通、per-stage pipeline 不引用 envelope flow、snapshot writer / compile entry 未有 capability 包裝）；(2)「⛽ Runtime Cost & Token Budget Guardrails」5 條 engineering discipline（reusable helper not one-shot script / shell wrapper not domain logic / workflow YAML 重用既有 capability / smoke 分層 / module map first grep second）；(3) 兩章節共同強化 P3 收斂 governance。`scripts/cap-entry.sh` 加 `[Supervisor Orchestration]` 區塊單行 hint 指向 ARCHITECTURE 章節 + boundary memo。**範圍嚴格守住**：純 docs + entry hint；不動 engine / schemas / scripts / smoke 邏輯；不打 release tag；P3 #8 release gate smoke 留下輪。
- [x] **P3 #8 release gate smoke (current branch)**：新增 `tests/e2e/test-supervisor-orchestration-release-gate.sh` 端到端 e2e fixture 串接所有 P3 落地模組（envelope helpers → snapshot writer → compile from envelope → workflow binding）。5 cases / 36 assertions：(0) happy 完整鏈路全通（extract / validate / drift / xref / resolve / 4 件套寫入 / 9-key compile output / failure_routing_resolved 對齊）；(1) schema halt 缺 failure_routing → snapshot writer exit 41 仍寫四件套（Q1=A）/ compile raise schema validation；(2) drift halt envelope.task_id≠nested → snapshot exit 41 / compile raise drift；(3) xref halt overrides 含 phantom step_id → snapshot exit 0（xref 非 writer 層責任）/ compile raise xref；(4) `cap workflow bind supervisor-orchestration` 仍 binding_status: ready + required_unresolved=0。接入 smoke-per-stage 為 case 34（spec-pipeline e2e 之前），與既有 P3 #1-#7 fixtures（cases 17 / 30 / 31 / 32 / 33 + binding case 4）共同構成 P3 整段 release gate；full smoke-per-stage.sh 升至 37 step / 全綠。**範圍嚴格守住**：純 deterministic e2e（無 AI / 無 network）；不動 engine / schemas / capability allowlist；不接 envelope→ticket 鏈路；不接 runtime dispatcher 行為實作；不打 release tag。**P3 整段（#1-#8）至此正式收斂，envelope flow 的 producer / schema / runtime gate / storage / compile entry / failure routing resolver / docs / release gate 全部落地，下一步 P4 SupervisorOrchestrator 可從乾淨 baseline 開工。**
...
> **Phase 4 closeout (P3 #8, current branch)**：P3 整段（#1-#8）正式收斂。Producer 規範（agent-skills/01-supervisor-agent.md §3.8）+ envelope schema 嚴格 11 required（含 failure_routing）+ runtime validation hook（exit 41 schema-class executor）+ four-part snapshot writer（symmetric P2，validation fail 仍寫）+ envelope-driven compile entry（與 legacy 並行，雙路徑共存）+ failure routing resolver / xref helper（pure Python，runtime dispatcher 留 P5）+ minimal binding workflow YAML + ARCHITECTURE module map / guardrails / boundary memo + 端到端 release gate e2e。`scripts/workflows/smoke-per-stage.sh` 升至 **37 step / 全綠 / 0 skipped**。仍開放但不阻擋 Phase 5：(1) runtime dispatcher（halt / retry / route_back / escalate 真正執行）屬 P5 AgentSessionRunner；(2) envelope→Type C ticket 鏈路打通；(3) per-stage pipeline 與 envelope flow 整合；(4) snapshot writer / compile entry 的 capability + shell wrapper 包裝。
...
- [x] 實作 logger milestone runner（v0.22.0 P8 #5：`engine/logger_gate_runner.py` 純函式 + `step_runtime.py run-logger-gate` CLI；dedicated 輸入 `--workflow-result` / `--result-md` / `--archive-summary` / `--mode milestone|final`；對接 P7 workflow-result.json 做 schema 驗證並抽 metrics（final_state / 各 step counts / failure / artifact / session / promote candidate count）；archive-summary missing 走 mode-aware severity（milestone→warn、final→blocked）；測試覆蓋 13 cases / 58 assertions，10 個 envelope 全通過 round-trip validate-gate-result CLI）
...
- [x] 實作 fail route handling（v0.22.0 P8 #6：`engine/gate_result_consumer.py` 純函式 + `step_runtime.py consume-gate-result` CLI；schema-first validation 後翻譯為 runtime decision；v1 完整支援 halt / route_back / escalate；retry → retry_unsupported（與 P8 #8 控制面重疊，預留 tag）；none → defer_to_workflow_yaml；缺 fail_routing 或 route_back_to_step 走保守 halt + needs_supervisor=true；雙軌 audit 寫入 workflow.log（人類可讀）+ route-history.jsonl（結構化）；測試覆蓋 16 cases / 46 assertions，既有 4 producer + validate-gate-result + schema 共 275 assertions 零回歸）
- [x] 支援 rerun failed gate（v0.22.0 P8：`engine/gate_result_rerun.py` 純函式 + `step_runtime.py rerun-gate` CLI；fail/blocked 預設可重跑、pass/warn 需 `--force`；依 `envelope.gate_type` 分派到 watcher/security/qa/logger 對應 runner，從 envelope 還原 `*GateInput`（QA 從 metrics 還原 coverage_threshold、Logger 從 target_artifacts 位置慣例還原 dedicated paths + metrics.mode）；版本化輸出 `<step_id>-<seq>.gate-result.json` 預設 seq 從 2 起算，原始永不覆寫；雙軌 audit 寫 workflow.log + route-history.jsonl 帶 `decision=rerun_executed/skipped/unsupported_gate_type`；測試覆蓋 17 cases / 51 assertions，既有 7 個 sibling 測試 321 assertions 零回歸）
- [x] enforce halt-on-risk（v0.22.0 P8 #7：在 `gate_result_consumer.derive_decision` 入口加 halt-on-risk policy，`risk_level=high|critical` 強制 `decision=halt` + `needs_supervisor=true`，優先於所有 fail_routing 推薦；reason 保留 producer fail_routing.action 與 reason 作為 audit context；notes 加 `halt_on_risk` / `overrode_action:<action>` 結構化 tag；測試從 16 cases / 46 assertions 升至 26 cases / 78 assertions，既有 producer/validate/schema/consumer/rerun 8 個測試 404 assertions 全綠；P8 governance gates 8 個子項全部達成）
