...
- **必須實際寫檔（硬性要求）**：你必須以實際檔案系統寫入動作把 4 個必交付資產建立在當前執行專案的 `docs/design/` 目錄下。**嚴禁只在 stdout / handoff_summary 用 code block 或 diff 列出內容、或寫「建議落地」/「待後續決定」/「未寫入」等語意取代真實寫檔**。Claude UI step 在 v0.21.3 cross-provider parity run 觀察到此偏差，明確違反本條規範。
- **檔案儲存路徑**：你必須將檔案建立在當前執行專案的 `docs/design/` 目錄下。（⚠️ 嚴格禁止寫入 `agent-skills/`、`policies/` 等協議來源目錄）。如果該目錄不存在，請主動建立。
...
- `agent_id: 03-UI`
- `output_paths` 必須對應**已實際寫入**的檔案路徑；不接受「建議落地」、「待後續決定」、「未寫入」之類的占位語意。交回前自我審查：對每個 output_paths 條目逐一確認檔案存在。
