...
- **Phase 進度**：v0.22 GA 達成 **P0–P10 全段 closeout**（platform-level capabilities 跨 rc1–rc18 達 steady state，canonical 收斂見 [docs/cap/PLATFORM-CLOSEOUT-v0.22.md](docs/cap/PLATFORM-CLOSEOUT-v0.22.md)）；v0.23 harness checkpoint 收斂 **A0 agent-skills baseline + H1–H4 replay verifier / project skills snapshot / input snapshots**；v0.24 收斂安裝後 CLI 主路徑、provider doctor / fail-fast、非 project workflow list、default help surface、namespace shortcuts 與 fixture 穩定性；**v0.24.3** 加上 read-only run observability 觀察層（`cap workflow logs / watch / inspect` follow-up + `--step / --compact / --json / --cap-home`）並把 `cap` CLI 對外訊息英文化；**v0.24.4** 把觀察體驗推到 docker / kubectl 等級（status glyphs、failed-step `Next:` footer、ps tip、`logs --tail`、fuzzy unknown command、namespace 錯誤訊息一致化）；**v0.24.5** 把可發現性補齊（`cap help workflow` / `cap help observe` topic 頁、main help 加 `[Observe Runs]` 區塊、dispatcher 端 `--help` 顯示完整 flag、README 加常見除錯流程）；**v0.24.6** ship Phase 5 read-only filter wave（`watch --failed-only` / `watch --step <id>` / `logs --since <value>`，含相對與絕對時間戳）；**v0.24.7** ship Karpathy guardrails Phase 1 dogfood scaffold（shared-layer skill + smoke workflow + capability registration，雙 provider real-run 雙 PASS）並 default `CAP_HOME=~/.cap` 讓 cap workflow 系列 out-of-box 抓到 shared layer；**v0.24.8** ship Role / Skill Registry Phase 1 schema preparation（optional `kind` enum 區分 executable role / attachable guardrail，附 model memo，runtime 完全沒動）；**v0.24.9** ship Karpathy Phase 2 builtin promote（dogfood 後正式上 builtin baseline + 7 agent references，4 excluded agents 邊界由 fixture 釘住，`engine/runtime_binder.py` 沒動）；**v0.24.10** 收斂 Phase 2 integration-mode evidence（run #8 high-visibility smoke + run #9 production multi-step internalised framing，pinned visibility spectrum 作未來評估座標；token-cost 實證無 concern）；**v0.24.11** ship Role / Skill Registry Phase 3 user-imported role registration（baseline policy §3.5 normative contract + customization guide 場景 5 + schema Examples block + 19-case resolver / source-policy fixture，含 shared-layer 正反面測試；runtime 完全沒動，Phase 4 / 5 attachment 仍 deferred）；H5 / H6 / H7 deferred，等真實使用者 dogfood 反饋再開；下一條主線回到自然 dogfood，待真實使用者註冊 role 時觸發 Phase 4 attachment design
